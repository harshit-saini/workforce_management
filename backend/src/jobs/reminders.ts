...
// Status keys are organization-defined, so this scans org by org rather than
// filtering on a fixed set of literal status strings across all tenants.
export async function scanTaskReminders() {
  const now = new Date();
  const organizations = await prisma.organization.findMany({ select: { id: true } });

  for (const org of organizations) {
    const [doneKeys, activeKeys] = await Promise.all([
      getStatusKeysByCategory(org.id, ["DONE"]),
      getStatusKeysByCategory(org.id, ["ACTIVE"]),
    ]);
    ...
    const activeTasks = await prisma.task.findMany({
      where: {
        organizationId: org.id,
        assigneeId: { not: null },
        OR: [{ isRecurring: true }, { status: { in: activeKeys } }],
      },
      select: { id: true, title: true, organizationId: true, assigneeId: true },
    });
